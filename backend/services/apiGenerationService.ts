...
export class ApiGenerationService {
  ...
  static generateApiRoutes(requirements: any) {
  ...
  private static generateRoutesCode(requirements: any) {
...
export default router;`;
  ...
  private static generateControllersCode(requirements: any) {
...
export const delete${resourceName} = async (req: any, res: any) => {
  ...
  private static generateModelsCode(requirements: any) {
    const resourceName = requirements.resourceName;
...
${fields.map((field: any) => `  ${field.name}: ${field.type.replace('String', 'string').replace('Number', 'number').replace('Boolean', 'boolean')};`).join('\n')}
  createdAt: Date;
  updatedAt: Date;
}
